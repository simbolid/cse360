...
def position_control(target, start_time):
    ...
    print(f"{time.time() - start_time:.3f}       {dist:.3f}        {orientation_error:.3f}        {u.tolist()}")
...
def main():
    ...
    try:
        ...
        print('Time      Distance     Orientation Error')

        start_time = time.time()
        target = (TARGET_X, TARGET_Y)  # x spot

        while is_running:
            if ROBOT_ID in positions and ROBOT_ID in rotations:
                command = position_control(target, start_time)
